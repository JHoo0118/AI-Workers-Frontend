...
const NavItem: FC<NavItemProps> = ({ menu }) => {
  ...
  // ===================== MENU MEGAMENU =====================
  const renderMegaMenu = (menu: Menu) => {
    if (!menu.children) {
      return null;
    }
  ...
  const renderMegaMenuNavlink = (item: Menu) => {
    return (
  ...
  // ===================== MENU DROPDOW =====================
  const renderDropdownMenu = (menuDropdown: Menu) => {
  ...
  const renderDropdownMenuNavlinkHasChild = (item: Menu) => {
  ...
  const renderDropdownMenuNavlink = (item: Menu) => {
    return (
      <Link
  ...
  // ===================== MENU MAIN MENU =====================
  const renderMainItem = (item: Menu) => {
    return (
  ...
  switch (menu.type) {
    ...
    case "megaMenu":
      return renderMegaMenu(menu);
    default:
      return (
        <li className="menu-item flex-shrink-0">{renderMainItem(menu)}</li>
      );
  }
};
